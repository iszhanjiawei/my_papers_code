#!/usr/bin/env bash
# Same-clip reference protocol, Semantic-VAE Direct C2 + frozen CAM++ + TPCA.
set -euo pipefail
...
if [[ ! -f "$checkpoint" || -L "$checkpoint" ]]; then
    echo "Missing regular TPCA speaker-conditioned checkpoint: $checkpoint" >&2
    exit 1
fi
...
"$python_bin" -u src/aligndit/script/eval/infer_celebvdub_semantic_vae_s1.py \
    ...
    --step "$step" \
    --config src/aligndit/config/finetune_celebvdub_mm_c2_semantic_vae_direct_speaker_tpca_ctc003_warmup.yaml \
