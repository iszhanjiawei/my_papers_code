...
echo "Launching Semantic-VAE Direct-C2 + CAM++ tail6 + TPCA MM9..11: LR=5e-5, CTC 0@10k -> 0.03@30k, stop at 200k" >&2
exec env \
    ...
    "$python_bin" -u -m accelerate.commands.launch \
        ...
        --dynamo_backend no \
        --num_processes 4 \
        --main_process_port "${TRAIN_PORT:-29628}" \
        src/aligndit/script/train/finetune_semantic_vae_c2_direct_speaker_tpca.py \
        --config-name finetune_celebvdub_mm_c2_semantic_vae_direct_speaker_tpca_ctc003_warmup \
        "$@"
