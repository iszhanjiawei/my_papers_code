...
def main(model_cfg):
    ...
    model = CFM_VT(
        ...
        tpca_visual_ctc_lambda=model_cfg.model.tpca_visual_ctc_lambda,
        tpca_path_lambda=model_cfg.model.tpca_path_lambda,
    )

    trainer = SemanticVaeDirectC2SpeakerTPCATrainer(
        model,
    ...
    if trainer.is_main:
        ...
        contract = {
            ...
            "tpca": "last three MM blocks [9,10,11], four local MHA heads; 40Hz visual input / 80Hz CTC logits",
            "seed": int(model_cfg.seed),
            "tensorboard_logdir": str(Path("runs", exp_name).resolve()),
        }
        contract_path = save_dir / "speaker_tpca_training_contract.json"
