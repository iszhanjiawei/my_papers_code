...
def build_model(config_path: Path, checkpoint_path: Path, expected_step: int, device: torch.device) -> CFM_VT:
    ...
    model = CFM_VT(
        ...
        tpca_visual_ctc_lambda=float(config.model.get("tpca_visual_ctc_lambda", 0.03)),
        tpca_path_lambda=float(config.model.get("tpca_path_lambda", 0.01)),
    ...
    if getattr(model.transformer, "tpca_enabled", False):
        tpca_step = int(model.transformer.tpca_step.item())
        if tpca_step != expected_step:
            raise RuntimeError(f"EMA TPCA schedule step mismatch: {tpca_step} != {expected_step}")
...
def historical_setting1_text(text: str) -> str:
    ...
    return historical_setting1_prompt(text)[0]


def historical_setting1_prompt(text: str) -> tuple[str, int]:
    """Keep S1 text identical while locating the target occurrence for TPCA.

    The reference and target say the same words. The CTC posterior must align
    only the target copy to the target lips, excluding all prompt separators.
    Character tokenization preserves this prefix count exactly.
    """

    prompt_text = text
    if prompt_text and len(prompt_text[-1].encode("utf-8")) == 1:
        prompt_text += " "
    joined = prompt_text + " " + text
    target_start = len(prompt_text) + 1 + len(text) - len(text.lstrip())
    return joined, target_start
...
def run(args: argparse.Namespace) -> None:
    ...
    with torch.inference_mode():
        for row, speaker_embedding in tqdm(
            ...
            joined_text, target_text_start = historical_setting1_prompt(str(row["text"]))
            generated, _ = model.sample(
                ...
                prompt_text_lens=torch.tensor([target_text_start], device=device, dtype=torch.long),
                steps=args.nfe,
    ...
    summary = {
        ...
        "tpca": {
            "enabled": bool(getattr(model.transformer, "tpca_enabled", False)),
            "step": int(model.transformer.tpca_step.item())
            if getattr(model.transformer, "tpca_enabled", False)
            else None,
            "video_frame_rate": 40,
            "alignment_text": "target occurrence only; prompt text and separators excluded",
        },
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        / "config/finetune_celebvdub_mm_c2_semantic_vae_direct_speaker_tpca_ctc003_warmup.yaml",
    )
    parser.add_argument(
        "--cache-root",
        type=Path,
