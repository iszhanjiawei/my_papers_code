...
ein notation:
b - batch
n - audio sequence (40 Hz in the C2 Semantic-VAE configuration)
nt - text sequence
nv - video sequence (25→40 Hz cached features in the C2 configuration)
nw - raw wave length
d - dimension
...
class MMDiTBlock_VT(DiTCrossBlock):
    ...
    def forward(
        ...
        if tpca is None:
            ...
            # Reuse C2's original MHA Q/K/V and output projection. TPCA never
            # introduces Hunyuan dual-CA or sends text into the visual stream.
            ca_output, path_loss = apply_tpca_multihead_attention(
                self.cross_attn, norm_ca, text, text_mask, tpca
            )
...
class DiT_VT_MMDiT(DiT):
    def __init__(
        ...
        if self.tpca_enabled:
            # Construct the new branch last so the original C2 random parameter
            # initialization remains identical under the same seed. The aligner
            # keeps its own initialization rather than the DiT initializer.
