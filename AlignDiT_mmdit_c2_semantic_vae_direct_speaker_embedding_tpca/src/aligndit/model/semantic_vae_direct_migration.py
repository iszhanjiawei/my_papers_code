...
def migrate_s2c_ema_into_model(
    ...
    *,
    ...
    if getattr(model.transformer, "tpca_enabled", False):
        tpca_keys = {"transformer.tpca_step"} | {
            "transformer.tpca_aligner." + key
            for key in model.transformer.tpca_aligner.state_dict()
        }
        actual_tpca = {key for key in target_keys if key.startswith("transformer.tpca_")}
        if actual_tpca != tpca_keys or not tpca_keys.issubset(new_target):
            raise RuntimeError("S2c TPCA migration only permits the new aligner and step buffer")
        if int(target_state["transformer.tpca_step"].item()) != 0:
            raise RuntimeError("New TPCA training must initialize its completed step to zero")
    actual_counts = (
        len(source_state),
        len(target_state),
    ...
    expected_counts = (
        ...
        EXPECTED_TARGET_KEYS + int(has_speaker) + len(tpca_keys),
        EXPECTED_LOADED_KEYS,
        EXPECTED_IGNORED_SOURCE_KEYS,
        EXPECTED_NEW_TARGET_KEYS + int(has_speaker) + len(tpca_keys),
    )
