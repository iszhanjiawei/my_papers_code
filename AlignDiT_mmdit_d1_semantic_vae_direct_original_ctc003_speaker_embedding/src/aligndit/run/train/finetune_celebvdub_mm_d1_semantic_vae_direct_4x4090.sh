...
# Original D1 (6 MM + 12 audio blocks), Semantic-VAE + speaker, CTC warmup 10k->30k.
...
while IFS=',' read -r gpu_index memory_free; do
    gpu_index="${gpu_index// /}"
    memory_free="${memory_free// /}"
    if [[ "$gpu_index" -lt 4 && "$memory_free" -lt "${MIN_GPU_FREE_MB:-18000}" ]]; then
        echo "Insufficient headroom on GPU $gpu_index: ${memory_free} MiB free" >&2
        exit 1
    fi
done < <(nvidia-smi --query-gpu=index,memory.free --format=csv,noheader,nounits)

echo "Launching isolated original D1 + Semantic-VAE + CAM++ speaker: LR=5e-5, CTC 0@10k -> 0.03@30k, activation checkpointing OFF" >&2
exec env \
