...
class DiT_VT_MMDiT(DiT):
    def __init__(
        ...
        if self.speaker_proj is not None:
            # Preserve the D1 audio path at initialization. A second zero gate
            # would prevent the new projection from receiving gradients.
    ...
    def get_speaker_delta(self, speaker_embedding, t, *, drop_speaker=False):
        ...
        # Cached vectors are validated on CPU. Normalize in float32 without
        # introducing a device synchronization on every flow step.
