...
class SemanticVaeDirectD1CtcWarmupTrainer(SemanticVaeDirectD1Trainer):
    ...
    def _forward_diagnostics(self, loss, loss_components) -> dict[str, float]:
        model = self.accelerator.unwrap_model(self.model)
        if model.ctc_lambda != self.current_ctc_lambda:
            raise RuntimeError("Scheduled CTC weight did not reach the model")
        diagnostics = super()._forward_diagnostics(loss, loss_components)
        diagnostics["ctc_active"] = float(self.current_ctc_lambda > 0)
        return diagnostics
    ...
    def load_checkpoint(self):
        ...
        expected = {
            "experiment": "semantic_vae_original_d1_speaker_ctc003_warmup10k30k",
