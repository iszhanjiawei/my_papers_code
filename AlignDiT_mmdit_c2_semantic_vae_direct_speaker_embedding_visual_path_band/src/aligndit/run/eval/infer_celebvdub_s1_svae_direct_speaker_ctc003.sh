#!/usr/bin/env bash
# Same-clip reference protocol for this isolated visual-path-band snapshot.
set -euo pipefail
...
checkpoint_dir="${ROOT_PREFIX}/zjw524/projects/data/ckpts/AlignDiT_MMDiT_c2_svae_speaker_visual_path_band_ctc003_warmup10k30k_40hz_CelebVDub_char"
step="${CKPT_STEP:-200000}"
checkpoint="$checkpoint_dir/model_${step}.pt"
cfg_video="${CFG_VIDEO:-2.0}"
output_dir="${OUTPUT_DIR:-$checkpoint_dir/eval_s1_${step}_visual_path_band_cfgv${cfg_video}}"
...
"$python_bin" -u src/aligndit/script/eval/infer_celebvdub_semantic_vae_s1.py \
    ...
    --step "$step" \
    --config src/aligndit/config/finetune_celebvdub_mm_c2_svae_speaker_visual_path_band.yaml \
