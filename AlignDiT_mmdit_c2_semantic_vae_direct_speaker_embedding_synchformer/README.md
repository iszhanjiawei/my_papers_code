# 当前实验：Semantic-VAE + CAM++ + Synchformer

新增同步条件的权重、数据处理和训练说明请先阅读 [SYNCHFORMER.md](SYNCHFORMER.md)，实际执行状态见 [SYNCHFORMER_STATUS.md](SYNCHFORMER_STATUS.md)。本目录是独立复制的新实验，使用文档中的 `speaker_synchformer` 训练和推理入口。
