...
# Direct-C2 retraining with CTC disabled through 10k and linearly warmed to
# 0.01 at 30k.  All other settings, including LR=5e-5, match Direct-C2.
...
echo "Launching Semantic-VAE Direct-C2 CTC warmup: LR=5e-5, CTC 0@10k -> 0.01@30k" >&2
exec env \
    ...
    "$python_bin" -u -m accelerate.commands.launch \
        ...
        --num_processes 4 \
        --main_process_port 29576 \
        src/aligndit/script/train/finetune_semantic_vae_c2_direct_ctc_warmup.py \
        --config-name finetune_celebvdub_mm_c2_semantic_vae_direct_ctc_warmup
