...
from aligndit.model.synchformer_features import (
    canonical_clip_key,
    load_synchformer_feature,
    validate_synchformer_cache,
)
...
class SemanticVaeCelebVDubDataset(Dataset):
    ...
    preserves the original C2 diffusion training set; its existing
    ...
    def __init__(
        ...
        *,
        ...
        synchformer_cache_dir: str | Path | None = None,
        synchformer_dim: int = 768,
        synchformer_checkpoint_sha256: str | None = None,
    ):
        self.manifest_path = _regular_file(manifest_path, label="CelebV-Dub train manifest")
        if self.manifest_path.name != "train.jsonl":
            raise ValueError(
                "Direct-C2 must use the complete train.jsonl split rather than the CTC-filtered subset: "
        ...
        self.synchformer_cache_dir: Path | None = None
        self.synchformer_dim = synchformer_dim
        self.synchformer_checkpoint_sha256 = synchformer_checkpoint_sha256
        self.synchformer_contract: dict | None = None
        if synchformer_cache_dir is not None:
            if synchformer_dim != 768 or not synchformer_checkpoint_sha256:
                raise ValueError("Frozen Synchformer requires 768-D features and an expected checkpoint SHA256")
            sync_root = Path(synchformer_cache_dir).expanduser().absolute()
            if sync_root.is_symlink() or not sync_root.is_dir():
                raise FileNotFoundError(f"Synchformer cache root must be a regular directory: {sync_root}")
            self.synchformer_cache_dir = sync_root.resolve(strict=True)
            for record in self.records:
                self._synchformer_clip_key(record)
            coverage = validate_synchformer_cache(
                self.synchformer_cache_dir,
                expected_checkpoint_sha256=synchformer_checkpoint_sha256,
                records=self.records,
                expected_manifest_sha256=expected_inventory_sha256,
            )
            audited_keys = coverage.get("valid_keys", [])
            if len(audited_keys) != CELEBVDUB_INVENTORY_COUNT or len(set(audited_keys)) != CELEBVDUB_INVENTORY_COUNT:
                raise RuntimeError("Synchformer coverage must contain the full 79,826-clip train/test inventory")
            split_counts = {split: sum(key.startswith(split + "/") for key in audited_keys) for split in ("train", "test")}
            if split_counts != {"train": expected_record_count, "test": CELEBVDUB_INVENTORY_COUNT - expected_record_count}:
                raise RuntimeError(f"Unexpected Synchformer cache split coverage: {split_counts}")
            self.synchformer_contract = {
                "cache_dir": str(self.synchformer_cache_dir),
                # Audit timestamps/elapsed time change on every full re-audit.
                # Pin semantic coverage so revalidating an identical cache can resume.
                "audited_clip_keys_sha256": hashlib.sha256(
                    json.dumps(sorted(audited_keys), separators=(",", ":")).encode("utf-8")
                ).hexdigest(),
                "checkpoint_sha256": synchformer_checkpoint_sha256,
                "inventory_sha256": expected_inventory_sha256,
                "dimension": synchformer_dim,
                "preprocessing": coverage["preprocessing"],
                "split_counts": split_counts,
            }

    @staticmethod
    def _synchformer_clip_key(record: dict) -> str:
        key = canonical_clip_key(record["utterance_key"])
        if canonical_clip_key(record["audio_relative_path"]) != key:
            raise ValueError(f"Synchformer clip key differs from reference audio identity: {key}")
        if canonical_clip_key(record["video_relative_path"]) != key:
            raise ValueError(f"Synchformer clip key differs from AV-HuBERT video identity: {key}")
        if key.split("/", 1)[0] != record["split"]:
            raise ValueError(f"Synchformer clip key has the wrong split: {key}")
        return key

    def _load_synchformer_feature(self, record: dict) -> torch.Tensor:
        if self.synchformer_cache_dir is None:
            raise RuntimeError("Synchformer cache is not configured")
        return load_synchformer_feature(
            self.synchformer_cache_dir,
            self._synchformer_clip_key(record),
            expected_checkpoint_sha256=self.synchformer_checkpoint_sha256,
            expected_dim=self.synchformer_dim,
        )

    def audit_synchformer_cache(self) -> dict:
        """Read all training features and revalidate their per-clip identities."""
        total_tokens, min_tokens, max_tokens = 0, float("inf"), 0
        for record in self.records:
            feature = self._load_synchformer_feature(record)
            count = feature.shape[0]
            total_tokens += count
            min_tokens, max_tokens = min(min_tokens, count), max(max_tokens, count)
        return {
            "complete": True,
            ...
            "contract": self.synchformer_contract,
        }
    ...
    def __getitem__(self, index: int) -> dict:
        ...
        if self.synchformer_cache_dir is not None:
            result["sync_feat"] = self._load_synchformer_feature(record)
        return result
    ...
    def collate_fn(batch: list[dict]) -> dict:
        ...
        has_sync = ["sync_feat" in item for item in batch]
        if any(has_sync):
            if not all(has_sync):
                raise RuntimeError("sync_feat must be present for every sample in a batch")
            sync_lengths = torch.tensor([item["sync_feat"].shape[0] for item in batch], dtype=torch.long)
            if (sync_lengths < 8).any() or (sync_lengths % 8).any():
                raise ValueError("Synchformer token lengths must be positive multiples of 8")
            max_sync_length = int(sync_lengths.max())
            result["sync_feat"] = torch.stack([
                F.pad(item["sync_feat"], (0, 0, 0, max_sync_length - item["sync_feat"].shape[0]))
                for item in batch
            ])
            result["sync_lens"] = sync_lengths
