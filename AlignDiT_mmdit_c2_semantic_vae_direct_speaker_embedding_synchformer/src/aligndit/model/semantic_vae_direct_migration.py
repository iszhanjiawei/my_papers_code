"""Strict S2c EMA migration for the Semantic-VAE Direct-C2 experiment."""
...
EXPECTED_NEW_TARGET_KEYS = 400
# Exactly six new Synchformer tensors. The pretrained S2c contract remains
# unchanged; only these named, shape-checked target additions are permitted.
EXPECTED_SYNC_SHAPES = {
    "transformer.sync_pos_emb": (1, 8, 768),
    "transformer.sync_in.0.weight": (768, 768),
    "transformer.sync_in.0.bias": (768,),
    "transformer.sync_in.2.w1.weight": (2048, 768),
    "transformer.sync_in.2.w2.weight": (768, 2048),
    "transformer.sync_in.2.w3.weight": (2048, 768),
}
...
def migrate_s2c_ema_into_model(
    ...
    *,
    ...
    has_sync = getattr(model.transformer, "sync_dim", 0) > 0
    actual_sync_keys = {
        key for key in target_keys
        if key.startswith("transformer.sync_in.") or key == "transformer.sync_pos_emb"
    }
    expected_sync_keys = set(EXPECTED_SYNC_SHAPES) if has_sync else set()
    if actual_sync_keys != expected_sync_keys:
        raise RuntimeError(f"Unexpected Synchformer migration keys: {sorted(actual_sync_keys ^ expected_sync_keys)}")
    for key in expected_sync_keys:
        if key not in new_target or tuple(target_state[key].shape) != EXPECTED_SYNC_SHAPES[key]:
            raise RuntimeError(f"Synchformer migration requires new {key} with shape {EXPECTED_SYNC_SHAPES[key]}")
    for key in ("transformer.sync_pos_emb", "transformer.sync_in.2.w2.weight"):
        if has_sync and torch.count_nonzero(target_state[key]).item() != 0:
            raise RuntimeError(f"Synchformer migration requires zero-initialized {key}")
    sync_key_count = len(expected_sync_keys)
    actual_counts = (
        len(source_state),
        len(target_state),
    ...
    expected_counts = (
        ...
        EXPECTED_TARGET_KEYS + int(has_speaker) + sync_key_count,
        EXPECTED_LOADED_KEYS,
        EXPECTED_IGNORED_SOURCE_KEYS,
        EXPECTED_NEW_TARGET_KEYS + int(has_speaker) + sync_key_count,
    )
    if actual_counts != expected_counts:
        raise RuntimeError(
            f"S2c-to-Direct-C2 migration count mismatch: expected={expected_counts}, actual={actual_counts}"
        )
    ...
    return DirectC2MigrationReport(
