"""CelebV-Dub Setting 1 inference for Semantic-VAE C2 with optional CAM++ and frozen Synchformer.
...
def load_setting1_synchformer_features(
    config,
    records: list[dict[str, Any]],
) -> tuple[list[torch.Tensor | None], dict[str, Any] | None]:
    """Validate full-cache provenance and all selected target-video features."""
    sync_dim = int(config.model.arch.get("sync_dim", 0))
    if not sync_dim:
        return [None] * len(records), None
    datasets = config.datasets
    if sync_dim != 768 or sync_dim != int(datasets.get("synchformer_dim", 0)):
        raise ValueError("Synchformer model and dataset dimensions must both be 768")
    cache = datasets.get("synchformer_cache_dir")
    checkpoint_sha256 = datasets.get("synchformer_checkpoint_sha256")
    if not cache or not checkpoint_sha256:
        raise ValueError("Synchformer inference requires a complete cache and checkpoint identity")
    cache_dir = Path(cache).resolve(strict=True)
    coverage = validate_synchformer_cache(
        cache_dir,
        expected_checkpoint_sha256=checkpoint_sha256,
        records=records,
        expected_manifest_sha256=datasets.expected_inventory_sha256,
    )
    features, sources = [], []
    for row in records:
        key = canonical_clip_key(row["utterance_key"])
        if canonical_clip_key(row["audio_relative_path"]) != key or canonical_clip_key(row["video_relative_path"]) != key:
            raise ValueError(f"Synchformer target-video identity mismatch: {key}")
        feature = load_synchformer_feature(cache_dir, key, checkpoint_sha256, sync_dim)
        features.append(feature)
        sources.append({"utterance_key": row["utterance_key"], "tokens": feature.shape[0], "clip_key": key})
    return features, {
        "cache_dir": str(cache_dir),
        "checkpoint_sha256": str(checkpoint_sha256),
        "coverage_report_sha256": sha256_file(cache_dir / "coverage_report.json"),
        "inventory_sha256": coverage["inventory_sha256"],
        "preprocessing": coverage["preprocessing"],
        "condition": "target RGB video; zero prompt prefix; dropped jointly with video in CFG",
        "sources": sources,
    }
...
def run(args: argparse.Namespace) -> None:
    ...
    sync_features, sync_metadata = load_setting1_synchformer_features(config, records)
    ...
    with torch.inference_mode():
        for row, speaker_embedding, sync_feature in tqdm(
            zip(records, speaker_embeddings, sync_features), total=len(records), desc=f"Semantic-VAE S1 {args.step}"
        ):
            ...
            if speaker_embedding is not None:
                condition_kwargs["speaker_embedding"] = speaker_embedding.unsqueeze(0).to(
                    device=device, dtype=torch.float32
                )
            if sync_feature is not None:
                # S1 prompt and target have identical duration. Duplicate the
                # token timeline with a blank prompt prefix, like AV-HuBERT.
                # The model additionally masks projected sync in prompt positions.
                total_sync = torch.cat((torch.zeros_like(sync_feature), sync_feature), dim=0)
                condition_kwargs["sync_feat"] = total_sync.unsqueeze(0).to(device=device, dtype=torch.float32)
                condition_kwargs["sync_lens"] = torch.tensor([total_sync.shape[0]], device=device)
