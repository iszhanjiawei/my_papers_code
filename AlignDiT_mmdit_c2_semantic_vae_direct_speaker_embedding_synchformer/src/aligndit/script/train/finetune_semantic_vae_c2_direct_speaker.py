...
from aligndit.model.trainer_semantic_vae_direct_speaker_synchformer import (
    SemanticVaeDirectC2SpeakerSynchformerTrainer,
)
...
def main(model_cfg):
    ...
    sync_dim = int(model_arc.get("sync_dim", 0))
    if sync_dim:
        if sync_dim != 768 or int(model_cfg.datasets.get("synchformer_dim", 0)) != sync_dim:
            raise ValueError("Synchformer model and dataset dimensions must both be 768")
        if not model_cfg.datasets.get("synchformer_cache_dir"):
            raise ValueError("Synchformer training requires a complete frozen feature cache")
        if model_cfg.ckpts.logger != "tensorboard":
            raise ValueError("This experiment requires TensorBoard loss and Synchformer gradient logging")
    ...
    train_dataset = SemanticVaeCelebVDubDataset(
        ...
        synchformer_cache_dir=model_cfg.datasets.get("synchformer_cache_dir") if sync_dim else None,
        synchformer_dim=sync_dim or 768,
        synchformer_checkpoint_sha256=model_cfg.datasets.get("synchformer_checkpoint_sha256") if sync_dim else None,
    )
    ...
    trainer_cls = SemanticVaeDirectC2SpeakerSynchformerTrainer if sync_dim else SemanticVaeDirectC2SpeakerTrainer
    trainer = trainer_cls(
        model,
    ...
    if trainer.is_main:
        ...
        if sync_dim:
            contract["synchformer_condition"] = {
                "dimension": sync_dim,
                "cache_dir": str(model_cfg.datasets.synchformer_cache_dir),
                "checkpoint_sha256": str(model_cfg.datasets.synchformer_checkpoint_sha256),
                "fusion": "frozen 25-fps features, segment positions, zero-ended MLP, per-sample resampling",
                "cfg": "drop jointly with video; mask prompt positions",
                "cache_contract": train_dataset.synchformer_contract,
            }
        contract_path = save_dir / ("synchformer_training_contract.json" if sync_dim else "speaker_training_contract.json")
