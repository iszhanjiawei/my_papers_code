...
def format_prompt_and_target_text(prompt_text, target_text):
    """Preserve the historical text string and identify its target suffix.

    Separator whitespace belongs to the prefix, so TPCA aligns actual target
    characters with target video rather than aligning a leading separator.
    """
    if prompt_text and len(prompt_text[-1].encode("utf-8")) == 1:
        prompt_text += " "
    leading_target_space = len(target_text) - len(target_text.lstrip())
    return prompt_text + target_text, len(prompt_text) + leading_target_space
...
def get_inference_prompt_vt(
    ...
    prompt_text_lengths = [[] for _ in range(num_buckets)]

    def pack_prompt(bucket_i):
        prompt = (
            utts[bucket_i],
            ...
            torch.nn.utils.rnn.pad_sequence(total_videos[bucket_i], batch_first=True),
        )
        if include_tpca_spans:
            prompt += (prompt_text_lengths[bucket_i],)
        return prompt
    ...
    for utt, prompt_text, prompt_wav, gt_text, gt_wav in tqdm(metainfo, desc="Processing prompts..."):
        ...
        # Text
        joined_text, prompt_text_len = format_prompt_and_target_text(prompt_text, gt_text)
        # Keep the legacy duration heuristic's prompt prefix unchanged.
        if prompt_text and len(prompt_text[-1].encode("utf-8")) == 1:
        ...
        prompt_text_lengths[bucket_i].append(prompt_text_len)

        batch_accum[bucket_i] += total_mel_len

        if batch_accum[bucket_i] >= infer_batch_size:
            # print(f"\n{len(ref_mels[bucket_i][0][0])}\n{ref_mel_lens[bucket_i]}\n{total_mel_lens[bucket_i]}")
            prompts_all.append(pack_prompt(bucket_i))
...
def get_celebvdub_test_metainfo_s1(metalst, celebvdub_path):
    ...
    celebvdub_path: path to CelebVDub dataset root (containing audio/, text/, avhubert_video_feat/)
    Returns: list of (gen_utt, ref_txt, ref_wav, gen_txt, gen_wav)
    """
    f = open(metalst)
    lines = f.readlines()
    f.close()
    ...
    for line in lines:
        ...
        folder_name, clip_id = parts[0], parts[1]

        txt_path = os.path.join(celebvdub_path, "text", split, folder_name, clip_id + ".txt")
        if not os.path.exists(txt_path):
            continue
        txt = open(txt_path).readline().strip().lower()

        wav_path = os.path.join(celebvdub_path, "audio", split, folder_name, clip_id + ".wav")
        if not os.path.exists(wav_path):
            continue

        gen_utt = os.path.join(split, folder_name, clip_id)

        # Setting 1: ref = gen (same clip's GT audio)
        metainfo.append((gen_utt, txt, wav_path, " " + txt, wav_path))
...
def get_celebvdub_test(metalst, gen_wav_dir, gpus, celebvdub_path, eval_ground_truth=False):
    ...
    for line in lines:
        ...
        gt_wav = os.path.join(celebvdub_path, "audio", split, folder_name, clip_id + ".wav")
        gen_utt = os.path.join(split, folder_name, clip_id)
        ...
        test_set_.append((gen_wav, gt_wav, txt))
...
def run_emosim(args):
    ...
    for gen_wav, prompt_wav, truth in tqdm(test_set):
        ...
        emb_gen = torch.tensor(res_gen[0]["scores"]).unsqueeze(0)
