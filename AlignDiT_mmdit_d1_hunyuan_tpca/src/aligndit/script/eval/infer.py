...
from aligndit.script.eval.utils import get_celebvdub_test_metainfo_s1, get_inference_prompt_vt, get_lrs3_test_metainfo, load_vocoder
...
def main():
    ...
    if args.vocoder_path is not None:
        vocoder_local_path = args.vocoder_path
    elif mel_spec_type == "hifigan_16k":
    ...
    model = CFM_VT(
        ...
        tpca_visual_ctc_lambda=getattr(model_cfg.model, "tpca_visual_ctc_lambda", 0.03),
        tpca_path_lambda=getattr(model_cfg.model, "tpca_path_lambda", 0.01),
    ).to(device)
        ...
        for prompt in tqdm(prompts, disable=not accelerator.is_local_main_process):
            ...
            with torch.inference_mode():
                generated, _ = model.sample(
                    ...
                    video=total_videos,
                    lens=ref_mel_lens,
                    prompt_text_lens=torch.tensor(prompt_text_lens, dtype=torch.long, device=device),
                    steps=nfe_step,
