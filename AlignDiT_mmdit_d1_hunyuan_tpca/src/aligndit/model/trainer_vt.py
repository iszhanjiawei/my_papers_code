        ...
        for epoch in range(skipped_epoch, self.epochs):
            ...
            for batch in current_dataloader:
                with self.accelerator.accumulate(self.model):
                    ...
                    if hasattr(transformer, "set_tpca_step"):
                        # The schedule is defined by completed optimizer updates,
                        # including after restoring a child checkpoint.
                        transformer.set_tpca_step(global_update)
                ...
                if self.accelerator.sync_gradients:
                    if hasattr(transformer, "set_tpca_step"):
                        transformer.set_tpca_step(global_update + 1)
                    if self.is_main:
                        self.ema_model.update()
                        ema_transformer = self.ema_model.ema_model.transformer
                        if hasattr(ema_transformer, "set_tpca_step"):
                            ema_transformer.set_tpca_step(global_update + 1)
                        ...
                        with torch.inference_mode():
                            generated, _ = self.accelerator.unwrap_model(self.model).sample(
                                ...
                                prompt_text_lens=torch.tensor(
                                    [len(text_inputs[0]) + 1], device=mel_spec.device, dtype=torch.long
                                ),
                                steps=nfe_step,
