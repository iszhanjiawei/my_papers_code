...
from aligndit.model.tpca_attention import apply_tpca_audio_attention
...
class HunyuanDualTextCrossAttention(nn.Module):
    ...
    def forward(self, audio, video, text, text_mask=None, tpca=None):
        if text.shape[1] == 0:
        ...
        if text_mask is not None:
            ...
            # Torch 2.4 SDPA can produce NaNs for all-masked keys. A dummy
            # key makes the computation finite; its residual is cleared below.
            safe_text_mask = text_mask.clone()
            safe_text_mask[:, 0] |= empty_text
        ...
        path_loss = None
        if tpca is not None:
            audio_result, path_loss = apply_tpca_audio_attention(
                q_a, key, value, output[:, :, :audio.shape[1]], tpca
            )
            output = torch.cat((audio_result, output[:, :, audio.shape[1]:]), dim=2)
        if text_mask is not None:
            output = output.masked_fill(empty_text[:, None, None, None], 0.0)
        ...
        if text_mask is not None:
            # Also suppress projection bias for a completely absent condition.
        ...
        if tpca is not None:
            return audio_output, video_output, path_loss
...
class MMDiTBlock_VT(DiTCrossBlock):
    ...
    def joint_attn(self, norm_x, norm_v, mask=None, v_mask=None, rope=None, v_rope=None, return_path=False):
        ...
        if return_path:
            return out_a, out_v, (q_a.detach(), k_v.detach())
        return out_a, out_v
    ...
    def forward(
        ...
        joint = self.joint_attn(
            norm_x, norm_v, mask=mask, v_mask=v_mask, rope=rope, v_rope=v_rope,
            return_path=tpca is not None,
        )
        attn_x, attn_v = joint[:2]
        if tpca is not None:
            # Keep a per-block copy: checkpoint recomputation must not mutate
            # a context shared with another block or CFG branch.
            tpca = {**tpca, "audio_query": joint[2][0], "video_key": joint[2][1]}
        ...
        if self.text_attention_mode == "hunyuan_dual":
            ...
            cross_result = self.cross_attn(norm_ca, norm_v_ca, text, text_mask=text_mask, tpca=tpca)
            ca_output, v_ca_output = cross_result[:2]
            if v_mask is not None:
...
class DiT_VT_MMDiT(DiT):
    def __init__(
        ...
        tpca_layers=(3, 4, 5),
        tpca_local_heads=4,
        tpca_visual_hidden_dim=256,
        tpca_ctc_upsample_factor=2,
        tpca_bias_strength=1.0,
        tpca_prior_smoothing=0.1,
        tpca_warmup_steps=2000,
        tpca_ramp_steps=8000,
        tpca_query_chunk_size=128,
    ):
        if text_attention_mode not in {"audio_only", "hunyuan_dual"}:
            raise ValueError(f"unsupported text_attention_mode: {text_attention_mode!r}")
        super().__init__(
        ...
        self.tpca_enabled = bool(tpca_enabled)
        self.tpca_layers = tuple(tpca_layers)
        self.tpca_local_heads = int(tpca_local_heads)
        self.tpca_bias_strength = float(tpca_bias_strength)
        self.tpca_prior_smoothing = float(tpca_prior_smoothing)
        self.tpca_warmup_steps = int(tpca_warmup_steps)
        self.tpca_ramp_steps = int(tpca_ramp_steps)
        self.tpca_query_chunk_size = int(tpca_query_chunk_size)
        self._tpca_cached_alignment = None
        if self.tpca_enabled:
            if text_attention_mode != "hunyuan_dual":
                raise ValueError("TPCA currently requires text_attention_mode=hunyuan_dual")
            if not self.tpca_layers or len(set(self.tpca_layers)) != len(self.tpca_layers):
                raise ValueError("tpca_layers must be a nonempty set of distinct MM layer indices")
            if any(type(i) is not int or not 0 <= i < n_mm_layers for i in self.tpca_layers):
                raise ValueError("TPCA layers must refer to existing MM blocks")
            if not 0 < self.tpca_local_heads < heads:
                raise ValueError("TPCA requires both local heads and retained global text heads")
            if not 0 < self.tpca_prior_smoothing < 1 or self.tpca_bias_strength < 0:
                raise ValueError("Invalid TPCA prior smoothing/bias strength")
            if min(self.tpca_warmup_steps, self.tpca_ramp_steps) < 0 or self.tpca_query_chunk_size < 1:
                raise ValueError("Invalid TPCA schedule or query chunk size")
            self.register_buffer("tpca_step", torch.tensor(0, dtype=torch.long))
            self.tpca_aligner = OccurrenceCTCAligner(
                video_dim=video_dim, vocab_size=text_num_embeds,
                hidden_dim=tpca_visual_hidden_dim, upsample_factor=tpca_ctc_upsample_factor,
            )
    ...
    def set_tpca_step(self, step):
        if self.tpca_enabled:
            self.tpca_step.fill_(int(step))

    def tpca_scale(self):
        if not self.tpca_enabled:
            return 0.0
        elapsed = int(self.tpca_step.item()) - self.tpca_warmup_steps
        if elapsed < 0:
            return 0.0
        if self.tpca_ramp_steps == 0:
            return 1.0
        return min(1.0, elapsed / self.tpca_ramp_steps)

    def clear_cache(self):
        super().clear_cache()
        self._tpca_cached_alignment = None

    def _get_tpca_alignment(self, video, text, video_mask, text_mask, video_start, text_start, cache):
        # All inputs are fixed within one ODE solve. Include pointers/versions
        # to reject stale cache entries when a caller changes conditions.
        cacheable = True
        def signature(tensor):
            ...
            try:
                ...
                # Inference-mode tensors have no mutation counter. Compute
                # afresh rather than risk reusing stale condition posteriors.
                version = None
                cacheable = False
            return (tensor.data_ptr(), tuple(tensor.shape), version)
        cache_key = tuple(signature(tensor) for tensor in
                          (video, text, video_mask, text_mask, video_start, text_start))
        cache = cache and cacheable
        if cache and not self.training and self._tpca_cached_alignment is not None:
            previous_key, alignment = self._tpca_cached_alignment
            if cache_key == previous_key:
                return alignment
        alignment = self.tpca_aligner(
            video, text, video_mask, text_mask, video_start=video_start, text_start=text_start,
            compute_loss=self.training and not cache,
        )
        if cache and not self.training:
            self._tpca_cached_alignment = (cache_key, alignment)
        return alignment
    ...
    def forward(
        ...
        cache: bool = False,
        tpca_text_start=None,
        tpca_video_start=None,
    ):
        ...
        tpca_aux = None
        if self.tpca_enabled:
            scale = self.tpca_scale()
            zero = x.new_zeros((), dtype=torch.float32)
            tpca_aux = {"ctc_loss": zero, "path_loss": zero, "feasible_fraction": zero,
                        "active_fraction": zero, "path_scale": scale,
                        "bias_strength": self.tpca_bias_strength * scale}
            if not (drop_text or drop_video):
                if text_mask is None:
                    text_mask = text != -1
                if video_mask is None:
                    video_mask = torch.ones(video.shape[:2], device=video.device, dtype=torch.bool)
                alignment = self._get_tpca_alignment(
                    video, text, video_mask, text_mask, tpca_video_start, tpca_text_start, cache
                )
                tpca_aux["ctc_loss"] = alignment["ctc_loss"]
                tpca_aux["feasible_fraction"] = alignment["feasible_fraction"]
                valid_video = video_mask.clone()
                if complementary_mask is not None:
                    valid_video &= ~complementary_mask[:, :video_len]
                if tpca_video_start is not None:
                    valid_video &= torch.arange(video_len, device=video.device)[None] >= tpca_video_start[:, None]
                local_text_mask = text_mask[:, :alignment["prior"].shape[-1] - 1].clone()
                if tpca_text_start is not None:
                    local_text_mask &= torch.arange(local_text_mask.shape[1], device=text.device)[None] >= tpca_text_start[:, None]
                active = generation_mask & alignment["feasible_mask"][:, None] & valid_video.any(dim=1)[:, None]
                tpca_aux["active_fraction"] = active.sum().float() / generation_mask.sum().clamp_min(1)
                if scale > 0:
                    tpca_context = {
                        "posterior": alignment["prior"], "video_mask": valid_video,
                        "text_mask": local_text_mask, "query_mask": active,
                        "local_heads": self.tpca_local_heads, "query_chunk_size": self.tpca_query_chunk_size,
                        "prior_smoothing": self.tpca_prior_smoothing,
                        "bias_strength": self.tpca_bias_strength * scale,
                    }
        ...
        if cfg_infer:  # pack cond & uncond forward: b n d -> 2b/3b n d
            ...
            (
                ...
                video_mask,
                complementary_mask,
            ) = [m.repeat(rep_n, 1) if m is not None else None for m in masks_to_repeat]
            generation_mask = generation_mask.repeat(rep_n, 1) if generation_mask is not None else None
            if tpca_context is not None:
                # Full/TTS/null are concatenated by branch, not by sample.
                # Only the first (full-condition) branch can use TPCA.
                tpca_context = dict(tpca_context)
                for key in ("posterior", "video_mask", "text_mask", "query_mask"):
                    value = tpca_context[key]
                    tpca_context[key] = torch.cat([value] + [torch.zeros_like(value)] * (rep_n - 1), dim=0)
        ...
        for layer_i, block in enumerate(self.transformer_blocks):
            ...
            block_tpca = tpca_context if layer_i in self.tpca_layers else None
            if self.checkpoint_activations:
                ...
                if is_mm:
                    ...
                    x, v = block_result[:2]
                    if block_tpca is not None:
                        path_losses.append(block_result[2])
                elif has_tail_text:
        ...
        if tpca_aux is not None:
            if path_losses:
                tpca_aux["path_loss"] = torch.stack(path_losses).mean()
            intermediates_ctc["__tpca__"] = tpca_aux
