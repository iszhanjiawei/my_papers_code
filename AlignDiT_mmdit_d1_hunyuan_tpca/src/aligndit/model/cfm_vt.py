...
class CFM_VT(CFM):
    def __init__(
        ...
        *args,
        ...
        audio_video_ratio=4,
        ctc_lambda=0.1,
        tpca_visual_ctc_lambda=0.03,
        tpca_path_lambda=0.01,
        **kwargs,
    ):
        ...
        self.tpca_visual_ctc_lambda = float(tpca_visual_ctc_lambda)
        self.tpca_path_lambda = float(tpca_path_lambda)
        if self.tpca_visual_ctc_lambda < 0 or self.tpca_path_lambda < 0:
            raise ValueError("TPCA auxiliary-loss weights must be nonnegative")
    ...
    def sample(
        ...
        *,
        ...
        prompt_text_lens: torch.Tensor | None = None,  # character prefix, including separators
        steps=32,
        cfg_strength=1.0,
        ...
        if lens.shape != (batch,) or torch.any(lens < 0) or torch.any(lens > cond_seq_len):
            raise ValueError("lens must contain one valid prompt-audio length per sample")
        ...
        tpca_enabled = bool(getattr(self.transformer, "tpca_enabled", False))
        tpca_kwargs = {}
        if prompt_text_lens is not None:
            prompt_text_lens = torch.as_tensor(prompt_text_lens, device=device)
            if prompt_text_lens.dtype not in (torch.int32, torch.int64):
                raise TypeError("prompt_text_lens must contain integer character-token counts")
            if (
                prompt_text_lens.shape != (batch,)
                or torch.any(prompt_text_lens < 0)
                or torch.any(prompt_text_lens >= text_lens)
            ):
                raise ValueError("prompt_text_lens must identify a nonempty target-text suffix in each sample")
        elif tpca_enabled:
            if torch.any(lens > 0):
                raise ValueError("TPCA sampling with reference audio requires explicit prompt_text_lens")
            prompt_text_lens = torch.zeros_like(text_lens)
        if tpca_enabled:
            if torch.any(lens % self.audio_video_ratio != 0):
                raise ValueError("TPCA prompt-audio lengths must end on the video-frame grid")
            tpca_kwargs = {
                "tpca_text_start": prompt_text_lens.to(torch.long),
                "tpca_video_start": lens // self.audio_video_ratio,
            }
        duration = torch.maximum(
            torch.maximum(text_lens, lens) + 1, duration
        )  # duration at least text/audio prompt length plus one token, so something is generated
        duration = duration.clamp(max=max_duration)
        if torch.any(duration <= lens):
            raise ValueError("max_duration must leave at least one generated frame after each audio prompt")
        ...
        if video is not None:
            max_video_len = (max_duration + self.audio_video_ratio - 1) // self.audio_video_ratio
        ...
        if batch > 1 or tpca_enabled:
            mask = duration_mask
            video_mask = mask[:, :: self.audio_video_ratio][:, : video.shape[1]]
        ...
        complementary_mask = cond_mask.squeeze(-1)[:, :: self.audio_video_ratio][:, : video.shape[1]]
        if exists(video_mask):
            complementary_mask &= video_mask

        if tpca_enabled:
            # The backbone deliberately rejects inference tensors as cache
            # keys because they have no mutation counter. Own ordinary,
            # immutable copies of just the alignment conditions for this
            # ODE solve; leave its states and other masks untouched.
            def cacheable_condition(tensor):
                if tensor is not None and torch.is_inference(tensor):
                    return tensor.detach().clone()
                return tensor

            with torch.inference_mode(False), torch.no_grad():
                video = cacheable_condition(video)
                text = cacheable_condition(text)
                video_mask = cacheable_condition(video_mask)
                text_mask = cacheable_condition(text_mask)
                tpca_kwargs = {name: cacheable_condition(value) for name, value in tpca_kwargs.items()}
    ...
    def forward(
        ...
        *,
        ...
        # TPCA auxiliary entries are not audio CTC heads. Remove the special
        # entry before the legacy CTC loop, including when audio CTC is off.
        tpca_aux = intermediates_ctc.pop("__tpca__", None)
        if tpca_aux is not None:
            visual_ctc_loss = tpca_aux["ctc_loss"]
            path_loss = tpca_aux["path_loss"]
            path_scale = float(tpca_aux.get("path_scale", 1.0))
            active = not (drop_text or drop_video)
            visual_weight = self.tpca_visual_ctc_lambda if active else 0.0
            path_weight = self.tpca_path_lambda * path_scale if active else 0.0
            weighted_visual_ctc = visual_ctc_loss * visual_weight
            weighted_path = path_loss * path_weight
            loss = loss + weighted_visual_ctc + weighted_path
            component_losses.update(
                tpca_visual_ctc_loss=float(visual_ctc_loss.detach()),
                tpca_path_loss=float(path_loss.detach()),
                tpca_visual_ctc_weighted_loss=float(weighted_visual_ctc.detach()),
                tpca_path_weighted_loss=float(weighted_path.detach()),
                tpca_visual_ctc_lambda=visual_weight,
                tpca_path_lambda=path_weight,
                tpca_path_scale=path_scale,
                tpca_active=float(active),
            )
            for name, value in tpca_aux.items():
                if name not in {"ctc_loss", "path_loss", "path_scale"}:
                    if torch.is_tensor(value):
                        if value.numel() != 1:
                            continue
                        value = value.detach().item()
                    if isinstance(value, (int, float)):
                        component_losses[f"tpca_{name}"] = float(value)
