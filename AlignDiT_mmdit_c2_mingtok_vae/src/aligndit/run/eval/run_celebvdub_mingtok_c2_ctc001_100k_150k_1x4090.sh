...
SWAY="${SWAY:-1.0}"
SWAY_TAG="$(printf '%s' "$SWAY" | sed -e 's/\.0*$//' -e 's/\./p/g')"
...
for step in $STEPS; do
    ...
    result_dir="${OUTPUT_ROOT}/${run_name}/celebvdub_test_s1/seed0_euler_nfe32_epss_sway${SWAY_TAG}_cfgt5_cfgv2_gt-dur"
    mkdir -p "$result_dir"
    ...
    if [ "$wav_count" -eq "$EXPECTED_ITEMS" ]; then
        ...
        run_logged "inference model_${step}" "logs/infer_celebvdub_mingtok_ctc001_sway${SWAY_TAG}_model_${step}.log" \
    ...
    for task in sim wer emosim; do
        ...
        run_logged "$task model_${step}" "logs/eval_celebvdub_mingtok_ctc001_sway${SWAY_TAG}_model_${step}_${task}.log" \
            env CUDA_VISIBLE_DEVICES="$GPU_ID" OMP_NUM_THREADS=1 PYTHONUNBUFFERED=1 PYTHONPATH=src \
    ...
    if [ "$av_feat_count" -eq "$EXPECTED_ITEMS" ]; then
        ...
        run_logged "AV-HuBERT extraction model_${step}" \
            "logs/eval_celebvdub_mingtok_ctc001_sway${SWAY_TAG}_model_${step}_avhubert_extract.log" \
    ...
    if metric_complete "$avsync_file" "AVSYNC"; then
        ...
        run_logged "avsync model_${step}" "logs/eval_celebvdub_mingtok_ctc001_sway${SWAY_TAG}_model_${step}_avsync.log" \
            env CUDA_VISIBLE_DEVICES="$GPU_ID" OMP_NUM_THREADS=1 PYTHONUNBUFFERED=1 PYTHONPATH=src \
            "$PYTHON_BIN" -u src/aligndit/script/eval/eval_celebvdub_test.py \
    ...
    summary_path="${result_dir}/_summary.txt"
    {
        ...
        echo "sampling=EMA seed0 euler nfe32 epss sway${SWAY} cfg_text5 cfg_video2 gt_duration"
