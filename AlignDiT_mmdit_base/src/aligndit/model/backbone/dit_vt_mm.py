...
class MMDiTBlock_VT(DiTCrossBlock):
    ...
    def forward(self, x, v, t, mask=None, v_mask=None, rope=None, v_rope=None, text=None, text_mask=None):
        ...
        # text cross attention (audio stream as query) 3、文本 cross-attention 只有音频流作为query，只有音频流作为query 文本不进入视频流
        ca_output, _ = self.cross_attn(
            x, text, text, key_padding_mask=~text_mask if text_mask is not None else None, need_weights=False
        )
        x = x + ca_output
