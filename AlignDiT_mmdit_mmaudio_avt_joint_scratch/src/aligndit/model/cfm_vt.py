...
class CFM_VT(CFM):
    ...
    def sample(
        ...
        *,
        ...
        self.eval()
        # A failed or externally interrupted previous ODE solve must not leak
        # text features into this request.
        ...
        if torch.any(duration.remainder(self.audio_video_ratio) != 0):
            raise ValueError(
                f"duration must be divisible by audio_video_ratio={self.audio_video_ratio}; "
                f"got {duration.tolist()}"
            )
        if torch.any(lens.remainder(self.audio_video_ratio) != 0):
            raise ValueError(
                f"prompt lens must be divisible by audio_video_ratio={self.audio_video_ratio}; "
                f"got {lens.tolist()}"
            )
        ...
        max_video_len = int((max_duration // self.audio_video_ratio).item())
        if video.shape[1] < max_video_len:
            raise ValueError(
                f"video provides {video.shape[1]} tokens, but duration requires {max_video_len}"
            )
        ...
        if batch > 1:
            mask = duration_mask
            # Video remains at its native frame rate. Build a prefix mask with
            # the actual video-token length instead of expanding the 100 Hz
            # audio mask to an invalid 4T sequence.
            video_lens = torch.div(duration, self.audio_video_ratio, rounding_mode="floor")
            video_mask = lens_to_mask(video_lens, length=video.shape[1])
        ...
        complementary_mask = complementary_mask[:, : video.shape[1]]
    ...
    def forward(
        ...
        *,
        ...
        expected_audio_lens = video_lens * self.audio_video_ratio
        if not torch.equal(lens, expected_audio_lens):
            raise ValueError(
                f"lens must equal video_lens * audio_video_ratio ({self.audio_video_ratio}); "
                f"got lens={lens.tolist()} and expected={expected_audio_lens.tolist()}"
            )
