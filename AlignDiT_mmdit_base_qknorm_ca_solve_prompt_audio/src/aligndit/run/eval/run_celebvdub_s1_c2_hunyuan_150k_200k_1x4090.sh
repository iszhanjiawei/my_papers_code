...
# the C2 Hunyuan dual text-CA / all-head RoPE checkpoints at 150k and 200k.
...
EXP_NAME=finetune_celebvdub_mm_c2_hunyuan_dual_ca_allrope
CKPT_DIR="${ROOT_PREFIX}/zjw524/projects/data/ckpts/AlignDiT_MMDiT_C2_HunyuanDualCA_AllRoPE_12MM6A_finetune_hifigan_16k_CelebVDub_char"
EXPECTED_ITEMS=213
...
evaluate_checkpoint() {
    ...
    PYTHONPATH=src "$PYTHON_BIN" -u src/aligndit/script/eval/verify_celebvdub_results.py \
        "$result_dir" > "$result_dir/verification.json"
    echo "Verified all 213 WAVs, features and four metrics: $result_dir/verification.json"
}
