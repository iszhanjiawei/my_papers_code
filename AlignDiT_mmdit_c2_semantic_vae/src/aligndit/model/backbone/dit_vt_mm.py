...
class DiT_VT_MMDiT(DiT):
    def __init__(
        ...
        # Runtime-only diagnostics. These are deliberately plain attributes rather
        # than buffers so text-context stabilization adds no checkpoint state keys.
    ...
    def _normalize_text_context(
        self,
        text_context: torch.Tensor,
        text_mask: torch.Tensor | None,
    ) -> tuple[torch.Tensor, torch.Tensor]:
        """Apply parameter-free per-token LayerNorm before every text CA."""
        ...
        if text_mask is None:
            ...
            if text_mask.ndim != 2 or text_mask.shape[0] != batch:
                raise ValueError(
                    f"text_mask must have shape [batch, tokens] with batch={batch}, got {tuple(text_mask.shape)}"
                )
            if text_mask.device != text_context.device:
                raise ValueError(f"text_mask must be on {text_context.device}, got {text_mask.device}")
            if text_mask.shape[1] < context_len:
                raise ValueError(
                    f"text_mask length {text_mask.shape[1]} is shorter than text context length {context_len}"
                )
    ...
    def forward(
        ...
        # The same parameter-free, padding-safe context is consumed by every
        # text cross-attention block. Normalizing once avoids layer-dependent
        # context scale drift while preserving checkpoint schema compatibility.
        text_embed, text_mask = self._normalize_text_context(text_embed, text_mask)
