...
def migrate_s2c_ema_into_model(
    ...
    *,
    ...
    # The speaker snapshot adds one zero-initialized tensor; its REPA child adds
    # exactly six MLP tensors. Keep the S2c contract strict for every other key.
    ...
    if has_repa:
        ...
        if actual_repa_keys != set(expected_repa_shapes) or any(
            tuple(target_state[key].shape) != shape for key, shape in expected_repa_shapes.items()
        ):
            raise RuntimeError(
                "S2c REPA migration requires a new 768->2048->2048->768 three-layer projector"
            )
    actual_counts = (
        len(source_state),
        len(target_state),
    ...
    expected_counts = (
        ...
        EXPECTED_TARGET_KEYS + int(has_speaker) + 6 * int(has_repa),
        EXPECTED_LOADED_KEYS,
        EXPECTED_IGNORED_SOURCE_KEYS,
        EXPECTED_NEW_TARGET_KEYS + int(has_speaker) + 6 * int(has_repa),
    )
