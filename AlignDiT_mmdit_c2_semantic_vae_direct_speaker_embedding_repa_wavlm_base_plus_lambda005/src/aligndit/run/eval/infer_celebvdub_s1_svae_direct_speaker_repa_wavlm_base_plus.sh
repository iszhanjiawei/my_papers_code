#!/usr/bin/env bash
# Same-clip S1 inference for the WavLM-Base+ REPA training variant.
set -euo pipefail
...
checkpoint_dir="${ROOT_PREFIX}/zjw524/projects/data/ckpts/AlignDiT_MMDiT_qknorm_ca_c2_semantic_vae_direct_speaker_repa_wavlm_base_plus_lambda005_40hz_CelebVDub_char"
step="${CKPT_STEP:-200000}"
checkpoint="$checkpoint_dir/model_${step}.pt"
cfg_video="${CFG_VIDEO:-2.0}"
output_dir="${OUTPUT_DIR:-$checkpoint_dir/eval_s1_${step}_speaker_repa_baseplus_cfgv${cfg_video}}"
if [[ ! -f "$checkpoint" || -L "$checkpoint" ]]; then
    echo "Missing regular REPA checkpoint: $checkpoint" >&2
    exit 1
fi
...
"$python_bin" -u src/aligndit/script/eval/infer_celebvdub_semantic_vae_s1.py \
    ...
    --step "$step" \
    --config src/aligndit/config/finetune_celebvdub_mm_c2_semantic_vae_direct_speaker_repa_wavlm_base_plus.yaml \
