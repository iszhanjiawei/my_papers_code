...
def main(args, custom_utils=None):
    ...
    for path in tqdm.tqdm(iterator, total=num):
        rel = os.path.relpath(os.path.abspath(path), abs_input)
        save_path = os.path.join(abs_output, os.path.splitext(rel)[0] + ".npy")
    ...
    logger.info(f"finished successfully: ok={n_ok}, skip={n_skip}, fail={n_fail}")
...
if __name__ == "__main__":
    ...
    sys.path.insert(0, args.user_dir)
