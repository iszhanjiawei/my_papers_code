...
class Trainer_VT(Trainer):
    ...
    def _clip_gradients(self) -> float | None:
        """Preserve default clipping while allowing experiment diagnostics."""
    ...
    def train(self, train_dataset: Dataset, num_workers=16, resumable_with_seed: int = None):
        ...
        if run_until_update is not None and start_update > run_until_update:
            raise RuntimeError(f"checkpoint update {start_update} is beyond requested run limit {run_until_update}")
