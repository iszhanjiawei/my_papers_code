...
- optional speaker identity is added to timestep conditioning only from
  `speaker_condition_start_layer` in the text-free audio tail
...
class DiT_VT_MMDiT(DiT):
    def __init__(
        ...
        if self.speaker_dim is None:
            ...
            if type(self.speaker_dim) is not int or self.speaker_dim <= 0:
                raise ValueError(f"speaker_dim must be a positive integer, got {self.speaker_dim}")
            ...
            if (
                type(self.speaker_condition_start_layer) is not int
                or not self.n_text_layers <= self.speaker_condition_start_layer < depth
            ):
                raise ValueError(
        ...
        if self.speaker_proj is not None:
            # Preserve the pretrained D1 audio path at initialization. Do not
            # add a second zero gate, which would block projection gradients.
    ...
    def forward(
        ...
        speaker_embedding: float["b ds"] | None = None,  # complete, unmasked reference waveform
        ...
        if drop_speaker is None:
            # Couple speaker identity to the final prompt-audio dropout.
        ...
        if cfg_infer:  # pack cond & uncond forward: b n d -> 2b/3b n d
            ...
            speaker_delta_list = []
            speaker_delta_cond = self.get_speaker_delta(speaker_embedding, t, drop_speaker=drop_speaker)
            if not (drop_text or drop_video):
                x_cond, text_embed_cond, v_cond = self.get_input_embed(
                    **embed_kwargs, drop_audio_cond=drop_audio_cond
                )
            ...
            # Match the branch-major torch.cat ordering for batch > 1.
            t = t.repeat((rep_n, 1))
            ...
            speaker_delta = self.get_speaker_delta(speaker_embedding, t, drop_speaker=drop_speaker)
