"""CelebV-Dub Setting 1 inference for the 64-D/40-Hz D1 model with CAM++.

Setting 1 uses the same clip's full GT audio as reference prompt and target.
Speaker conditioning comes from that exact reference waveform; this is not an
independent-reference speaker evaluation.
"""
...
def build_model(config, checkpoint_path: Path, expected_step: int | None, device: torch.device) -> tuple[CFM_VT, int]:
    arch = config.model.arch
    representation = config.model.audio_representation
    if (
        config.model.backbone != "DiT_VT_MMDiT"
        or int(arch.depth) != 18
    ...
    if (
        ...
        or int(arch.video_dim) != 1024
        or bool(arch.video_rope_scaled)
        or int(representation.channels) != LATENT_DIM
    ...
    if sha256_file(config.datasets.vocab_path) != config.datasets.expected_vocab_sha256:
        raise RuntimeError("The vocabulary differs from the training configuration")
    if arch.get("speaker_dim") is not None and (
        int(arch.speaker_dim) != 192 or int(arch.speaker_condition_start_layer) != 6
    ):
        raise RuntimeError("D1 speaker conditioning requires 192-D CAM++ at all audio-only blocks [6, 18)")
    ...
    model = CFM_VT(
        transformer=model_cls(**OmegaConf.to_container(arch, resolve=True), text_num_embeds=vocab_size, mel_dim=LATENT_DIM),
    ...
    expected_keys = {
        "ema_model_state_dict", "model_state_dict", "optimizer_state_dict", "scheduler_state_dict", "update"
    }
    if not isinstance(checkpoint, dict) or set(checkpoint) != expected_keys:
        raise RuntimeError("Expected a Direct-D1 training checkpoint with model, EMA, optimizer, scheduler and update")
    checkpoint_step = checkpoint["update"]
    if not isinstance(checkpoint_step, int) or isinstance(checkpoint_step, bool) or checkpoint_step < 0:
        raise RuntimeError(f"Invalid checkpoint update: {checkpoint_step!r}")
    ...
    if any(key not in {"initted", "step"} and not key.startswith("ema_model.") for key in ema_state):
        raise RuntimeError("Unexpected keys in EMA checkpoint state")
    selected = {
    ...
    return model.eval().requires_grad_(False).to(device=device, dtype=torch.float32), checkpoint_step
...
def load_setting1_speaker_embeddings(
    ...
    """Validate all selected vectors and bind them to the exact S1 prompt.

    No speaker extractor is run during inference. The frozen cache is read
    from original reference waveforms, never generated/VAE-reconstructed audio.
    """
    ...
    for row in records:
        ...
        if (
            ...
            or not audio_relative_path.parts
        ...
        if not prompt_waveform.is_file():
            raise FileNotFoundError(f"S1 reference waveform is missing: {prompt_waveform}")
        embeddings.append(
    ...
    return embeddings, {
        ...
        "conditioned_blocks_zero_based": list(range(6, 18)),
        "sources": sources,
    }
...
def run(args: argparse.Namespace) -> None:
    ...
    args.cache_root = args.cache_root or Path(config.datasets.cache_root)
    args.normalization = args.normalization or Path(config.datasets.normalization_path)
    if sha256_file(args.normalization) != config.datasets.expected_normalization_sha256:
        raise RuntimeError("The latent normalization differs from the training configuration")
    if args.nfe <= 0:
    ...
    model, checkpoint_step = build_model(config, args.checkpoint.resolve(strict=True), args.step, device)
    ...
    with torch.inference_mode():
        for row, speaker_embedding in tqdm(
            zip(records, speaker_embeddings), total=len(records), desc=f"Semantic-VAE D1 speaker S1 {checkpoint_step}"
        ):
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--step", type=int, help="Optional expected update; otherwise read it from the checkpoint")
    ...
    parser.add_argument(
        "--cache-root",
        type=Path,
        help="Defaults to datasets.cache_root in --config",
    )
    parser.add_argument(
        "--normalization",
        type=Path,
        help="Defaults to the pinned datasets.normalization_path in --config",
    )
