#!/usr/bin/env bash
# D1 Semantic-VAE + CAM++: EMA inference and historical WER / SIM / EMOSIM / AVSync.
# Usage: bash this_script.sh [150000|last|/path/to/model.pt] [new_output_directory]
# S1 uses the same full GT waveform as prompt and target, including speaker input.
...
config="${CONFIG:-src/aligndit/config/finetune_celebvdub_mm_d1_semantic_vae_direct_speaker_ctc003_warmup.yaml}"
checkpoint_dir="${CHECKPOINT_DIR:-${ROOT_PREFIX}/zjw524/projects/data/ckpts/AlignDiT_MMDiT_qknorm_ca_d1_semantic_vae_direct_speaker_ctc003_warmup10k30k_40hz_CelebVDub_char}"
selector="${1:-last}"
step_args=()
...
output_dir="${2:-$checkpoint_dir/eval_s1_speaker_$(basename "${checkpoint%.pt}")_seed0_nfe32_cfgt5_cfgv2}"
eval_gpu="${EVAL_GPU:-0}"
...
# Validate all S1 references AND frozen speaker vectors before expensive models.
PYTHONPATH=src "$python_bin" - "$test_list" "$celebvdub" "$output_dir" "$config" <<'PY'
import sys
...
test_list, root, output, config_path = map(Path, sys.argv[1:])
...
config = load_composed_config(config_path.resolve(strict=True))
records = load_test_records(Path(config.datasets.cache_root), test_list)
embeddings, metadata = load_setting1_speaker_embeddings(config, records)
if metadata is None or len(embeddings) != 213 or any(value is None for value in embeddings):
    raise RuntimeError("This entry requires complete speaker-conditioned S1 inference")
print("Preflight passed: 213/213 reference inputs and prompt speaker vectors; empty output directory")
PY

echo "[1/6] D1 Semantic-VAE speaker inference: $checkpoint -> $output_dir"
...
if not summary.get("speaker_embedding") or len(summary["speaker_embedding"]["sources"]) != 213:
    raise RuntimeError("Missing per-prompt speaker provenance")
for clip in clips:
...
for metric in ("sim", "wer", "emosim", "avsync"):
    ...
    print(lines[-1])
print(f"Complete: 213/213 waveforms, features, speaker sources and metric rows; EMA update {summary['checkpoint']['update']}")
PY
